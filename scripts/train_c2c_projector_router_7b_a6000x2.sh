...
if [[ "${SKIP_STAGE1:-0}" != "1" && ! -s "$stage1_llm/config.json" ]]; then
  ...
  run_ds icl_stage1_query_rec_training.py \
    ...
    --wandb_tags "$WANDB_TAGS,stage1" \
    --wandb_run_name "c2c-p123-stage1-qwen25-7b-a6000x2" \
...
run_ds icl_stage2_routing_training.py \
  ...
  --wandb_project "$WANDB_PROJECT" \
  --wandb_mode "$WANDB_MODE" \
  --wandb_tags "$WANDB_TAGS,stage2" \
  --wandb_run_name "c2c-p123-stage2-qwen25-7b-a6000x2" \
